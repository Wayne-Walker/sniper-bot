...
class LeaderboardFetcher:
    ...
    def fetch(self) -> tuple[List[WalletRank], List[WalletRank]]:
        ...
        # Sort by PnL for the chosen window. The PnL lives inside each row's
        # "windowPerformances" list (not a top-level field), so we sort by the
        # same extractor used to build WalletRank — otherwise every row reads 0
        # and the winner/loser split is effectively random.
        sorted_by_pnl = sorted(filtered, key=self._extract_pnl, reverse=True)
