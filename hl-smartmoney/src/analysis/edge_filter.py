...
class EdgeFilter:
    ...
    def _save_signals(self, signals: List[TradeSignal]) -> None:
        ...
        try:
            ...
            log.error(f"Failed to save JSON signals: {e}")
        ...
        try:
            ...
            log.error(f"Failed to save TXT signals: {e}")
    ...
    def _format_txt(self, signals: List[TradeSignal], now_str: str) -> str:
        ...
        for i, s in enumerate(signals, 1):
            ...
            smart_dir       = "LONG"  if s.winner_net_notional > 0 else "SHORT"
            dumb_dir        = "LONG"  if s.loser_net_notional  > 0 else "SHORT"

            lines += [
                f"  [{i}]  ${s.coin}  --  {direction_label}  --  {s.net_sentiment}",
                SEP2,
                ...
                "",
                SEP3,
                f"  TRADE PLAN   : {direction_label} ${s.coin} @ ${s.mark_price:,.4f}",
                f"  EDGE         : Smart money {smart_dir} vs Dumb money {dumb_dir}",
    ...
    def _alert(self, signals: List[TradeSignal]) -> None:
        ...
        if not signals:
            return
        if os.environ.get("SUPPRESS_EDGE_ALERT") == "1":
            return
